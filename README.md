...
├── AgoraHub/     stevenvo780/agora-hub             → systemd edu-hub.service en stev-server
...
| **AgoraHub** | TermiCoop Hub: socket.io server que coordina los workers Docker (terminales, sesiones SSH) y expone HTTP `/agent/*` para que el agente IA ejecute comandos en workers. | systemd `edu-hub.service` (user) en `stev-server:3010`. Carpeta `ops/` con docker-compose de NAS. |
| **AgoraWorker** | Worker Docker que corre por workspace (`edu-worker-<wsId>`). Da terminal Linux + handler `agent-command`. Incluye el daemon `worker-host-sync/` que mantiene `/workspace/<wsId>/` espejado contra MinIO+Firestore. | Imagen `stevenvo780/edu-worker:latest` en DockerHub. Daemon `agora-host-sync.service` en stev-server. Carpeta `desplieges-prod/` con scripts deploy. |
...
| Deploy hub a stev-server | `cd AgoraWorker/desplieges-prod && ./deploy_hub.sh` |
...
- **Backend IA**: `https://agora-backend-578238159459.us-central1.run.app` (rewrite desde `agora.elenxos.com/api/agora-ai/stream`).
- **Hub TermiCoop**: `stev-server:3010`, systemd `edu-hub.service`.
- **Workers**: 27 contenedores Docker `edu-worker-*` administrados por `agora-host-sync.service`.
